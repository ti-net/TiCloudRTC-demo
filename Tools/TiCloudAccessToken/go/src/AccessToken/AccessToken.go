...
func CreateAccessToken(enterpriseId, token, userId string) AccessToken {
	ts := uint32(time.Now().Unix()) + 24*3600
	salt := uint32(random(1, 99999999))
	message := make(map[uint16]uint32)
	return AccessToken{enterpriseId, token, userId, ts, salt, message, "", 0, ""}
}
...
func (token *AccessToken) FromString(originToken string) bool {
	...
	originContent := originToken[(VERSION_LENGTH + ENTERPRISE_ID_LENGTH):]
	originContentDecoded, err := base64.StdEncoding.DecodeString(originContent)
	if err != nil {
		return false
	}

	signature_, crc_uid_, msg_raw_content_, err := unPackContent(originContentDecoded)
	if err != nil {
		return false
	}
...
func (token *AccessToken) Build() (string, error) {
	...
	ret = version + token.EnterpriseId + base64.StdEncoding.EncodeToString(bytes_content)
	return ret, nil
}
...
func unPackContent(buff []byte) (string, uint32, string, error) {
