package com.tinet.accessToken;

import java.io.ByteArrayOutputStream;
import java.io.IOException;
import java.util.TreeMap;

/**
 * @Author: zhoujiang
 * @Date: 2022/7/25 16:30
 */
public class AccessToken {
    public enum Privileges {
        /**
         * Privileges
         */
        API(1);

        public short intValue;

        Privileges(int value) {
            intValue = (short) value;
        }
    }

    private static final String VER = "001";

    public String enterpriseId;
    public String token;
    public String userId;
    public byte[] signature;
    public byte[] messageRawContent;
    public int crcUid;
    public PrivilegeMessage message;

    public AccessToken(String enterpriseId, String token, String userId) {
        this.enterpriseId = enterpriseId;
        this.token = token;
        this.userId = userId;
        this.crcUid = 0;
        this.message = new PrivilegeMessage();
    }

    public AccessToken(String enterpriseId, String token, String userId, int salt, int ts) {
        this.enterpriseId = enterpriseId;
        this.token = token;
        this.userId = userId;
        this.crcUid = 0;
        this.message = new PrivilegeMessage(salt, ts);
    }

    public String build() throws Exception {

        messageRawContent = Utils.pack(message);
        signature = generateSignature(enterpriseId, token, userId, messageRawContent);
        crcUid = Utils.crc32(userId);

        PackContent packContent = new PackContent(signature, crcUid, messageRawContent);
        byte[] content = Utils.pack(packContent);
        return getVersion() + this.enterpriseId + Utils.base64Encode(content);
    }

    public void addPrivilege(Privileges privilege, int expireTimestamp) {
        message.messages.put(privilege.intValue, expireTimestamp);
    }

    public static String getVersion() {
        return VER;
    }

    public static byte[] generateSignature(String enterpriseId, String token, String userId, byte[] message) throws Exception {

        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        try {
            baos.write(enterpriseId.getBytes());
            baos.write(userId.getBytes());
            baos.write(message);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return Utils.hmacSign(token, baos.toByteArray());
    }

    public static class PrivilegeMessage implements PackableEx {
        public int salt;
        public int ts;
        public TreeMap<Short, Integer> messages;

        public PrivilegeMessage() {
            salt = Utils.randomInt();
            ts = Utils.getTimestamp() + 24 * 3600;
            messages = new TreeMap<>();
        }

        public PrivilegeMessage(int salt, int ts) {
            this.salt = salt;
            this.ts = ts;
            messages = new TreeMap<>();
        }

        @Override
        public ByteBuf marshal(ByteBuf out) {
            return out.put(salt).put(ts).putIntMap(messages);
        }

        @Override
        public void unmarshal(ByteBuf in) {
            salt = in.readInt();
            ts = in.readInt();
            messages = in.readIntMap();
        }
    }

    public static class PackContent implements PackableEx {
        public byte[] signature;
        public int crcUid;
        public byte[] rawMessage;

        public PackContent() {
            // Nothing done
        }

        public PackContent(byte[] signature, int crcUid, byte[] rawMessage) {
            this.signature = signature;
            this.crcUid = crcUid;
            this.rawMessage = rawMessage;
        }

        @Override
        public ByteBuf marshal(ByteBuf out) {
            return out.put(signature).put(crcUid).put(rawMessage);
        }

        @Override
        public void unmarshal(ByteBuf in) {
            signature = in.readBytes();
            crcUid = in.readInt();
            rawMessage = in.readBytes();
        }
    }
}
